# ARCHITECTURE.md

## Overview

Document the current architecture for `flyto-health-twin` here. Include runtime boundaries, major modules, data flow, external integrations, and deployment surfaces.

## Frontend Surfaces

If this repo contains UI, every screen must follow the Flyto2 Frontend Quality Gate in `AGENTS.md`.

## Update Rule

Update this file when module boundaries, storage, APIs, deployment, or frontend structure changes.
